...
export default function InacoResultPage() {
  ...
  const handlePrint = async () => {
    if (!resultImageUrl || !qrValue || isPrinting) return;
    setPrintError(null);
    setIsPrinting(true);
    ...
    <main className="inaco-landscape-shell relative isolate flex min-h-dvh w-full items-center justify-center overflow-hidden px-4 py-8 text-white">
      ...
      <div className="flex w-[85%] max-w-2xl flex-col items-center gap-5 xl:w-[45%]">
        ...
        <div className="flex w-full flex-col gap-3">
          {resultImageUrl ? (
            <button type="button" onClick={() => void handlePrint()} disabled={isPrinting} className="block">
              <Image
      ...
      {isPrinting ? (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/70">
          <div className="flex w-[86%] max-w-2xl flex-col items-center gap-3 rounded-2xl bg-white p-8 text-black">
            ...
            <p className="text-center text-[3vw] font-medium">Menyiapkan print... Mohon tunggu.</p>
          </div>
        </div>
      ) : null}
      ...
      <style jsx global>{`
        @media print {
          ...
          body > *:not(#print-hires-only-inaco) {
            display: none !important;
          }
          #print-hires-only-inaco {
